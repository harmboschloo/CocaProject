...
void wxPropertyGrid::Init1()
{
    ...
    // added by Harm
    m_propClick = (wxPGProperty*) NULL;
    m_propDrag = (wxPGProperty*) NULL;
    ...
    if ( p )
    {
        ...
        if ( x >= marginEnds )
        {
            ...
            // Click on value.
                ...
                // added by Harm
                m_propClick = ( columnHit == 1 ? (wxPGProperty*) NULL : p );
...
bool wxPropertyGrid::HandleMouseMove( int x, unsigned int y, wxMouseEvent &event )
{
    ...
    else
    {
        ...
        // added by Harm, dragging support
        if ( event.Dragging() && event.LeftIsDown() )
        {
            if ( m_propClick && !m_propDrag  )
            {
                m_propDrag = m_propClick;
                SendEvent( wxEVT_PG_ITEM_DRAGGING, m_propDrag );
            }
        }
        else
        {
...
// added by Harm
DEFINE_EVENT_TYPE( wxEVT_PG_ITEM_DRAGGING )
